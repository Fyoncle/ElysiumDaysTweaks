package net.fyoncle.elysiumcore.utility.constants;

import net.minecraft.util.Identifier;

public class Textures {
    // - Menu Logos -
    // Default Logo
    public static final Identifier ED_DEFAULT_LOGO = new Identifier("elysium-core",
            "logos/eddefaultlogo.png");
    // Halloween Logo
    public static final Identifier ED_HALLOWEEN_LOGO = new Identifier("elysium-core",
            "logos/edhalloweenlogo.png");
    // Christmas Logo
    public static final Identifier ED_CHRISTMAS_LOGO = new Identifier("elysium-core",
            "logos/edchristmaslogo.png");


    // - Menu Panoramas -
    public static final Identifier ED_DEFAULT_PANORAMA = new Identifier("elysium-core",
            "panoramas/default_panorama.png");
    public static final Identifier ED_HALLOWEEN_PANORAMA = new Identifier("elysium-core",
            "panoramas/halloween_panorama.png");
    public static final Identifier ED_CHRISTMAS_PANORAMA = new Identifier("elysium-core",
            "panoramas/christmas_panorama.png");


    // - Health Bar Textures -
    // OFF
    public static Identifier UNFOCUSED_OFF_HEALTHBAR_TEXTURE = new Identifier(
            "elysium-core",
            "textures/gui/neat_disabled_button_unfocused.png");
    public static Identifier FOCUSED_OFF_HEALTHBAR_TEXTURE = new Identifier(
            "elysium-core",
            "textures/gui/neat_disabled_button_focused.png");
    // ON
    public static Identifier UNFOCUSED_ON_HEALTHBAR_TEXTURE = new Identifier(
            "elysium-core",
            "textures/gui/neat_enabled_button_unfocused.png");
    public static Identifier FOCUSED_ON_HEALTHBAR_TEXTURE = new Identifier(
            "elysium-core",
            "textures/gui/neat_enabled_button_focused.png");
    // - Discord Button Textures -
    public static Identifier DISCORD_BUTTON_UNFOCUSED_TEXTURE = new Identifier(
            "elysium-core",
            "textures/gui/discord_button_unfocused.png");
    public static Identifier DISCORD_BUTTON_FOCUSED_TEXTURE = new Identifier(
            "elysium-core",
            "textures/gui/discord_button_focused.png");
    // - Wiki Button Textures -
    public static Identifier WIKI_BUTTON_UNFOCUSED_TEXTURE = new Identifier(
            "elysium-core",
            "textures/gui/wiki_button_unfocused.png");
    public static Identifier WIKI_BUTTON_FOCUSED_TEXTURE = new Identifier(
            "elysium-core",
            "textures/gui/wiki_button_focused.png");


    // - Ram Warning Screen Textures -
    // Open Guide Button
    public static Identifier OPEN_GUIDE_BUTTON_UNFOCUSED = new Identifier(
            "elysium-core",
            "textures/gui/open_guide_button_unfocused.png");
    public static Identifier OPEN_GUIDE_BUTTON_FOCUSED = new Identifier(
            "elysium-core",
            "textures/gui/open_guide_button_focused.png");
    // Ignore Button
    public static Identifier IGNORE_BUTTON_UNFOCUSED = new Identifier(
            "elysium-core",
            "textures/gui/ignore_button_unfocused.png");
    public static Identifier IGNORE_BUTTON_FOCUSED = new Identifier(
            "elysium-core",
            "textures/gui/ignore_button_focused.png");
    // Don't Show Again Button
    public static Identifier DONT_SHOW_AGAIN_BUTTON_UNFOCUSED = new Identifier(
            "elysium-core",
            "textures/gui/dont_show_again_button_unfocused.png");
    public static Identifier DONT_SHOW_AGAIN_BUTTON_FOCUSED = new Identifier(
            "elysium-core",
            "textures/gui/dont_show_again_button_focused.png");


    // - Other -
    // Default Button
    public static Identifier DEFAULT_BUTTON_UNFOCUSED = new Identifier(
            "elysium-core",
            "textures/gui/default_button_unfocused.png");
    public static Identifier DEFAULT_BUTTON_FOCUSED = new Identifier(
            "elysium-core",
            "textures/gui/default_button_focused.png");
    // Green Button Texture
    public static Identifier GREEN_BUTTON_UNFOCUSED_TEXTURE = new Identifier(
            "elysium-core",
            "textures/gui/green_button_unfocused.png");
    public static Identifier GREEN_BUTTON_FOCUSED_TEXTURE = new Identifier(
            "elysium-core",
            "textures/gui/green_button_focused.png");
}